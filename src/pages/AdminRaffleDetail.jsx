...
export default function AdminRaffleDetail() {
  ...
  const exportSales = () => {
    ...
    const totalArrecadado = approvedSales.reduce((sum, s) => sum + Number(s.total_amount || 0), 0);
                  ...
                  approvedSales.map((sale) => (
                    <div key={sale.id} className="bg-slate-700/30 rounded-lg p-4">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className="text-white font-semibold">R$ {Number(sale.total_amount || 0).toFixed(2)}</span>
                      </div>
